...
class Albom:
    ...
    def __init__(self, basedir, namebook=None, async=False, name=None):
        self.name = name if name else basedir
        self.basedir = os.path.abspath(basedir)
        if not os.path.exists(basedir):
            os.makedirs(basedir)
        self.namebook = os.listdir(basedir) if namebook==None else namebook
    ...
    def __getitem__(self, key):
        return Model(os.path.join(self.basedir,self.namebook[key])) 
    ...
    def add(self, filepath):
        ...
        with open(os.path.join(newdir,'note.json'), "w") as write_file:
            json.dump(local_data, write_file, indent=4)
